...
from utils.adversarial_trainer import train_epoch_adversarial, val_epoch_adversarial
...
def objective(args, single_trial):
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        # Train one epoch
        epoch_loss_seg, epoch_loss_adv = train_epoch_adversarial(
            model,  # opt_model
            train_loader,
            optimizer,
            criterion,
        ...
        if logger is not None:
            logger.log({"train/loss_seg": epoch_loss_seg}, epoch)
            logger.log({"train/loss_adv": epoch_loss_adv}, epoch)
            print(f"Train Epoch {epoch}, loss seg {epoch_loss_seg}, loss adv {epoch_loss_adv}")
        if epoch % args.check_val_every_n_epoch == 0:
            # Val one epoch
            val_loss, accuracy, surface, other_metrics = val_epoch_adversarial(
                model,  # opt_model
                val_loader,
                criterion,
                args.device,
