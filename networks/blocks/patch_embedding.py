...
from networks.layers.utils import get_norm_layer
        ...
        if norm_type is not None:
            ...
            if self.norm_type == "layer":
                if isinstance(norm_type, Tuple):
                    norm_type[1]["normalized_shape"] = embed_dim
                else:
                    norm_type = (norm_type, {"normalized_shape": embed_dim})

            self.norm = get_norm_layer(name=norm_type,
                                       spatial_dims=spatial_dims,  # This is same as spatial dimensions
                                       channels=embed_dim)
        else:
            self.norm = None
