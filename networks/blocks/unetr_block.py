...
from ..blocks.dynunet_block import UnetBasicBlock, UnetResBlock, get_conv_layer
...
class UnetrUpBlock(nn.Module):
    ...
    def forward(self, inp, skip, modalities=None):
        # number of channels for skip should equals to out_channels
        out = self.transp_conv(inp)
        out = torch.cat((out, skip), dim=1)
        out = self.conv_block(out, modalities)
        return out
    ...
    def forward(self, x, modalities=None):
        ...
        for blk in self.blocks:
            if self.conv_block:
                # Convolution
                x = blk[0](x)
                # UnetResBlock or UnetBasicBlock with modalities if instance_cond
                x = blk[1](x, modalities)
            else:
                x = blk(x)
        return x
...
class UnetrBasicBlock(nn.Module):
    ...
    def forward(self, inp, modalities=None):
        return self.layer(inp, modalities)
